  ...
  async submitSummary(clerkId: string, url: string): Promise<SummaryResult> {
    ...
    if (existing?.summary) {
      ...
      return {
        ...
        channelName: existing.channelName ?? undefined,
    ...
    // Fetch video metadata (non-critical)
    const details = await this.transcriptService.fetchVideoDetails(videoId);
    ...
    if (details) {
      ...
      video.channelName = details.channelName;
    ...
    return {
      ...
      summary,
      truncated,
      title: savedVideo.title ?? undefined,
      thumbnailUrl: savedVideo.thumbnailUrl ?? undefined,
      channelName: savedVideo.channelName ?? undefined,
