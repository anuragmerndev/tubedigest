  ...
  return (
    <header className="h-[52px] border-b border-border shrink-0 flex items-center px-5 gap-3 bg-background">
      ...
      <div className="flex-1 flex items-center gap-1.5 text-[13px] text-td-text-muted min-w-0">
        {crumbs.map((crumb, i) => (
          <span key={i} className="flex items-center gap-1.5">
            {i > 0 && (
              <ChevronRight size={12} className="text-td-text-dim shrink-0" />
            )}
            <span
              className={
                i === crumbs.length - 1
      ...
      <div className="relative" ref={menuRef}>
        ...
        {menuOpen && (
          <div className="absolute right-0 top-[calc(100%+6px)] z-50 w-52 rounded-lg border border-border bg-card shadow-xl py-1">
            <div className="px-3 py-2 border-b border-border">
              <div className="text-[12px] text-td-text-muted truncate">
                {userName}
              </div>
            </div>
            <button
